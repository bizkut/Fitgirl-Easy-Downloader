...
class FitGirlDownloaderApp:
    ...
    def setup_ui(self):
        ...
        self.queue_tree.column("status", width=450)
    ...
    def _download_worker(self):
        while True:
            ...
            def prepare_downloads():
                try:
                    for idx, link in enumerate(links):
                        ...
                        try:
                            with batch_lock:
                                should_show_preparing = not batch_state['started_downloading']
                            if should_show_preparing:
            ...
            while True:
                ...
                try:
                    plan_item = download_queue.get(timeout=0.5)
                except queue.Empty:
                    continue
    ...
    def _poll_torrent_status(self):
        ...
        for tree_id, torrent_info in list(self.torrent_queue_items.items()):
            ...
            # Build detailed status string for the queue treeview
